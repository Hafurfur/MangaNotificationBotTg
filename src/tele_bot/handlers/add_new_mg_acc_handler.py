...
def add_manga_acc(callback_query: CallbackQuery) -> None:
    ...
    if _check_expired_mess(state, callback_query):
        ...
        try:
            ...
            log.error('Ошибка при удалении сообщения (Telebot)', exc_info=error)
        except Exception as error:
...
def _check_expired_mess(state: StateContext, callback_query: CallbackQuery) -> bool:
    log.debug(f'Проверка истекло ли действие сообщения | state={state.data}')
...
def _send_manga_account(callback_query: CallbackQuery, len_list_acc: int, acc_id: str, acc_name: str, photo_id: str,
                        index: int) -> None:
    log.debug(f'Отправка сообщения с результатом поиска манга аккаунта | '
              f'len_list_acc={len_list_acc}, acc_id={acc_id}, acc_name={acc_name}, photo_id={photo_id}, index={index}')
